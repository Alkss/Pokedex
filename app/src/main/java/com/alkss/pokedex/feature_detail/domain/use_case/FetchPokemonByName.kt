...
class FetchPokemonByName @Inject constructor(
    private val remoteRepository: PokemonRemoteRepository,
    private val localRepository: PokemonLocalRepository
) {

    suspend operator fun invoke(pokemonName: String): PokemonWithTypeAndMoves? {
        val list = localRepository.getPokemonByName(pokemonName)
    ...
    private suspend fun getPokemonFromApi(name: String): PokemonWithTypeAndMoves? {
